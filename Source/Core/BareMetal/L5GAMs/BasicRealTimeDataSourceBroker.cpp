/**
 * @file BasicRealTimeDataSourceBroker.cpp
 * @brief Source file for class BasicRealTimeDataSourceBroker
 * @date 21/mar/2016
 * @author pc
 *
 ...
 * the class BasicRealTimeDataSourceBroker (public, protected, and private). Be aware that some 
...
BasicRealTimeDataSourceBroker::BasicRealTimeDataSourceBroker() :
        ReferenceContainer() {
    application = NULL_PTR(RealTimeApplication *);
    synchronized = false;
    pollingSem = NULL_PTR(FastPollingMutexSem *);
    finalised = false;
}

void BasicRealTimeDataSourceBroker::SetApplication(RealTimeApplication &rtApp) {
    application = &rtApp;
}

bool BasicRealTimeDataSourceBroker::AddVariable(ReferenceT<RealTimeDataDefI> def,
                                                void * const ptr) {
...
bool BasicRealTimeDataSourceBroker::AddVariablePrivate(ReferenceT<RealTimeDataDefI> def,
                                                       void * ptr,
                                                       void * memStart) {
...
void *BasicRealTimeDataSourceBroker::GetData(const uint32 i) const {
...
void *BasicRealTimeDataSourceBroker::GetMemoryPointer(const uint32 n) const {
    void *ret = NULL_PTR(void*);
...
void *BasicRealTimeDataSourceBroker::GetMemoryPointerPrivate(const uint32 n) const {
...
bool BasicRealTimeDataSourceBroker::Finalise() {
    ...
    if (ret) {
        ...
        for (uint32 i = 0u; i < numberOfDS; i++) {
            ...
            if (ret) {
                ...
                ret = (dsDef != NULL);
                if (ret) {
                    FastPollingMutexSem *tempSem = dsDef->GetPollingSemaphore();
                    if (tempSem != NULL) {
                        if(!synchronized) {
                            pollingSem=tempSem;
                            synchronized=true;
...
bool BasicRealTimeDataSourceBroker::IsSync() const {
    return synchronized;
}
