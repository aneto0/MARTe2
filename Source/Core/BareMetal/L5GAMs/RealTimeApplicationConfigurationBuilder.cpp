    ...
    if (ret && (!foundANode)) {
        ...
        if ((ret) && (signalTypeDefined)) {
            ...
            if (signalTypeDescriptor == InvalidType) {
                ...
                //if (ret) {
                uint32 nn = 0u;
                StreamString fAlias = alias;
                for (nn = 0u; (nn < numberOfElements) && (ret); nn++) {
                    isStructuredData = true;
                    StreamString fsignalName = signalName;
                    if (numberOfElements > 1u) {
                        fsignalName = "";
                        (void) fsignalName.Printf("%s[%d]", signalName, nn);
                        fAlias = "";
                        (void) fAlias.Printf("%s[%d]", alias.Buffer(), nn);
                    }
                    ...
                    if (ret) {
                        ...
                        ret = SignalIntrospectionToStructuredData(signalDatabase, signalType.Buffer(), fsignalName.Buffer(),
                                                                  ...
                                                                  triggerSignalName.Buffer(), fullType.Buffer(), ranges, samples, frequency,
                                                                  trigger, resolvedSignal, signalNumber, syncSet, triggerSet,
                                                                  isFunctionsDatabase);
                    }
                    if (ret) {
                        ret = syncSet;
    ...
    if (ret) {
        ...
        for (i = 0u; (i < numberOfMembers) && (ret); i++) {
            ...
            if (isStructured) {
                ...
                uint32 nOfElements = entry.GetNumberOfElements(0u);
                uint32 nn = 0u;
                StreamString tempName = fullSignalName;
                StreamString tempAlias = fullAliasName;
                for (nn = 0u; (nn < nOfElements) && (ret); nn++) {
                    if (nOfElements > 1u) {
                        fullSignalName = "";
                        (void) fullSignalName.Printf("%s[%d]", tempName.Buffer(), nn);
                        if (tempAlias.Size() > 0u) {
                            fullAliasName = "";
                            (void) fullAliasName.Printf("%s[%d]", tempAlias.Buffer(), nn);
                        }
                    }
                    ...
                    ret = SignalIntrospectionToStructuredData(signalDatabase, entry.GetMemberTypeName(), fullSignalName.Buffer(),
                                                              ...
                                                              typeNameStr.Buffer(), ranges, samples, frequency, trigger, data, signalNumber,
                                                              syncSet, triggerSet, isFunctionDatabase);
                }
            }
            else {
                if (ret) {
