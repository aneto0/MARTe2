...
RealTimeApplicationConfigurationBuilder::RealTimeApplicationConfigurationBuilder(
		ConfigurationDatabase &globalDatabaseIn,
		const char8 * const defaultDataSourceNameIn) {
	defaultDataSourceName = defaultDataSourceNameIn;
	...
	if (!ret) {
		REPORT_ERROR_FULL(ErrorManagement::FatalError,
				"In RealTimeApplicationConfigurationBuilder failed to MoveToRoot");
	}
...
bool RealTimeApplicationConfigurationBuilder::InitialiseSignalsDatabaseFromConfiguration() {
	...
	if (ret) {
		...
		ret = SearchDataSources(globalDatabase, dataSourcesDatabase, path,
				index, timingDataSourceCounter);
		if (ret) {
			ret = (timingDataSourceCounter == 1u);
			if (timingDataSourceCounter > 1u) {
				REPORT_ERROR(ErrorManagement::InitialisationError,
						"Only one TimingDataSource per application allowed");
			} else {
	...
	if (ret) {
		...
		if (ret) {
			...
			for (i = 0u; (i < numberOfDataSources) && (ret); i++) {
				...
				const ClassProperties *prop = dataSource->GetClassProperties();
				if (ret) {
					ret = (prop != NULL);
				}
				if (ret) {
...
bool RealTimeApplicationConfigurationBuilder::FlattenSignalsDatabase(
				...
				while ((j < numberOfSignals) && (ret)) {
					...
					if (StringHelper::Compare(signalName, "Locked") != 0) {
						...
						if (ret) {
							...
							ret = FlattenSignal(isFunctionsDatabase, signalName,
									signalLocalDatabase, signalNumber);
						}
						if (ret) {
						...
						if (signalDatabase.Read("Locked", locked)) {
							...
							if (ret) {
								if (locked != 0u) {
									ret = signalDatabase.MoveToAncestor(1u);
									if (ret) {
										ret = signalDatabase.Write("Locked",
												locked);
									}
									if (ret) {
										ret = signalDatabase.MoveRelative(
												"Signals");
									}
								}
							}
...
bool RealTimeApplicationConfigurationBuilder::FlattenSignal(
		const bool isFunctionsDatabase, const char8 * const signalName,
		ConfigurationDatabase &resolvedSignal, uint32 &signalNumber) {
	bool ret = true;
	StreamString signalType;

	ConfigurationDatabase &signalDatabase =
			(isFunctionsDatabase) ? (functionsDatabase) : (dataSourcesDatabase);
	...
	if (!signalTypeDefined) {
		...
		for (n = 0u; (n < numberOfElements); n++) {
			...
			if (signalDatabase.MoveRelative(elementName.Buffer())) {
				if (StringHelper::Compare(elementName.Buffer(), "MemberAliases")
						!= 0) {
					if (StringHelper::Compare(elementName.Buffer(), "Defaults")
							!= 0) {
						foundANode = true;
						StreamString signalFullName;
						if (ret) {
							ret = signalFullName.Printf("%s.%s", signalName,
									elementName.Buffer());
						}

						if (ret) {
							ret = FlattenSignal(isFunctionsDatabase,
									signalFullName.Buffer(), resolvedSignal,
									signalNumber);
						}
						if (ret) {
							ret = signalDatabase.MoveToAncestor(1u);
	...
	if (ret && (!foundANode)) {
			...
			if (!signalDatabase.Read("DataSource", dataSourceName)) {
				if (defaultDataSourceName.Size() > 0u) {
					...
					REPORT_ERROR_PARAMETERS(
							...
							"DataSource not specified for %s and DefaultDataSource not specified",
							signalName)
				}
			}
		}
		...
		if ((ret) && (signalTypeDefined)) {
			TypeDescriptor signalTypeDescriptor =
					TypeDescriptor::GetTypeDescriptorFromTypeName(
							signalType.Buffer());
			...
			if (signalTypeDescriptor == InvalidType) {
				...
				if (signalDatabase.Read("NumberOfDimensions",
						numberOfDimensions)) {
				...
				if (ret) {
					...
					if (ret) {
						...
						ret = SignalIntrospectionToStructuredData(
								signalDatabase, signalType.Buffer(), signalName,
								alias.Buffer(), dataSourceName.Buffer(),
								syncSignalName.Buffer(), fullType.Buffer(),
								ranges, samples, frequency, resolvedSignal,
					...
					if (ret) {
						...
						if (!ret) {
							REPORT_ERROR_PARAMETERS(
									ErrorManagement::InitialisationError,
									"Invalid synchronising member specified in %s",
									signalName)
		...
		if ((ret) && (!isStructuredData)) {
			...
			const char8 *properties[] = { "Type", "NumberOfDimensions",
					"NumberOfElements", "Alias", "Ranges", "DataSource",
					"Samples", "Default", "Frequency", NULL_PTR(char8 *) };
			uint32 p = 0u;
		...
		for (i = 0u; (i < numberOfFunctions) && (ret); i++) {
			...
			if (exists) {
				...
				uint32 numberOfSignals =
						functionsDatabase.GetNumberOfChildren();
				for (uint32 s = 0u; (s < numberOfSignals) && (ret); s++) {
					...
					StreamString dataSourceName;
					if (ret) {
						if (!functionsDatabase.Read("DataSource",
								dataSourceName)) {
							dataSourceName = defaultDataSourceName;
						}
					}
					...
					if (ret) {
						ret = FindDataSourceNumber(dataSourceName,
								dataSourceNumber);
					}
					if (ret) {
						ret = dataSourcesDatabase.MoveAbsolute("Data");
					...
					if (ret) {
						ret = dataSourcesDatabase.MoveRelative(
								dataSourceNumber.Buffer());
					}
					if (ret) {
					...
					if (ret) {
						...
						ret = AddSignalToDataSource(functionName,
								dataSourceName);
						//}
					}
					if (ret) {
...
bool RealTimeApplicationConfigurationBuilder::FindDataSourceNumber(
		StreamString dataSourceName, StreamString &dataSourceNumber) {
	bool ret = dataSourcesDatabase.MoveAbsolute("Data");
	if (ret) {
		uint32 numberOfDataSources = dataSourcesDatabase.GetNumberOfChildren();
		bool done = false;
		for (uint32 n = 0u; (n < numberOfDataSources) && (ret) && (!done);
				n++) {
			...
			if (ret) {
				done = (StringHelper::Compare(qualifiedName.Buffer(),
						dataSourceName.Buffer()) == 0);
			}
			if (ret) {
...
bool RealTimeApplicationConfigurationBuilder::AddSignalToDataSource(
		StreamString functionName, StreamString dataSourceName) {
...
//Check if the signal already exists
	...
	for (n = 0u; (n < numberOfSignals) && (ret) && (!signalAlreadyExists);
			n++) {
		...
		if (ret) {
			ret = dataSourcesDatabase.Read("QualifiedName",
					dataSourceSignalName);
		}
		if (ret) {
			signalAlreadyExists = (StringHelper::Compare(signalName.Buffer(),
					dataSourceSignalName.Buffer()) == 0);
		}
		//Move to the next signal
		if (ret) {
			ret = dataSourcesDatabase.MoveToAncestor(1u);
		...
		for (n = 0u; (n < numberOfSignals) && (ret); n++) {
			...
			if (dataSourcesDatabase.Read("FullType", otherFullType)) {
				...
				if (sizeFullTypeOK && sizeOtherFullTypeOK) {
					...
					if (ret) {
						ret = CheckTypeCompatibility(fullType, otherFullType,
								signalName, dataSourceSignalName);
					}
				}
			}
			...
			if (ret) {
				...
				REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError,
						"The data source assigned to the signal %s in %s is incompatible",
	...
	if ((!signalAlreadyExists) && (ret)) {
		...
		if (ret) {
			...
			REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError,
					"Cannot add the signal %s in GAM %s because the related DataSource is locked",
					originalSignalName.Buffer(), functionName.Buffer())
		}
		if (ret) {
			ret = dataSourcesDatabase.Write("QualifiedName",
					signalName.Buffer());
		}
	}
	if (ret) {
		...
		const char8 *properties[] = { "Type", "NumberOfDimensions",
				"NumberOfElements", "Default", NULL_PTR(char8 *) };
		uint32 p = 0u;
		while ((properties[p] != NULL_PTR(char8 *)) && (ret)) {
			AnyType elementSignalDatabase = functionsDatabase.GetType(
					properties[p]);
			AnyType elementDataSourceDatabase = dataSourcesDatabase.GetType(
					properties[p]);
//Property already exists, check compatibility!
			if (elementSignalDatabase.GetTypeDescriptor() != VoidType) {
				if (elementDataSourceDatabase.GetTypeDescriptor() != VoidType) {
					...
					ret = functionsDatabase.Read(properties[p],
							sElementSignalDatabase);
					if (ret) {
						ret = dataSourcesDatabase.Read(properties[p],
								sElementDataSourceDatabase);
					}
					if (ret) {
						ret = (StringHelper::Compare(
								sElementSignalDatabase.Buffer(),
								sElementDataSourceDatabase.Buffer()) == 0);
					}
					if (!ret) {
						//Report mismatch!
						StreamString fullPropertyName;
						ret = fullPropertyName.Printf("%s.%s",
								originalSignalName.Buffer(), properties[p]);
						if (!ret) {
					...
					if (!isDsLocked) {
						ret = dataSourcesDatabase.Write(properties[p],
								elementSignalDatabase);
					}
				}
			}
			p++;
		...
		if (replace && ret) {
			...
			if (fullType.Size() > 0u) {
				if (ret) {
					ret = dataSourcesDatabase.Write("FullType",
							fullType.Buffer());
				}
			}
		}
			...
			while ((s < numberOfSignals) && (ret)) {
				...
				if (ret) {
					...
					if (!ret) {
						REPORT_ERROR_PARAMETERS(
								ErrorManagement::InitialisationError,
								"QualifiedName was not defined for signal at position: %s in %s",
				...
				if (ret) {
					...
					for (uint32 k = 0u;
							(k < numberOfSignals) && (!signalFound) && (ret);
							k++) {
						if (k != s) {
							char8 terminator;
							StreamString otherSNumber =
									dataSourcesDatabase.GetChildName(k);
							ret = dataSourcesDatabase.MoveRelative(
									otherSNumber.Buffer());
							...
							if (ret) {
								...
								StreamString token2;
								signalFound = true;
								while (signalNameStr.GetToken(token1, ".",
										terminator) && (signalFound) && (ret)) {
									if (otherSName.GetToken(token2, ".",
											terminator)) {
				...
				if (ret) {
					if (signalFound) {
						...
						if (ret) {
							...
							if (ret) {
								numberOfSignals =
										dataSourcesDatabase.GetNumberOfChildren();
								s--;
							...
							REPORT_ERROR_PARAMETERS(ErrorManagement::FatalError,
									"The name %s in %s is defined as a node",
									signalName.Buffer(),
									dataSourceName.Buffer())
						}
					} else {
						...
						if (ret) {
							...
							REPORT_ERROR_PARAMETERS(ErrorManagement::FatalError,
									"Undefined type for signal %s in %s",
									signalName.Buffer(),
									dataSourceName.Buffer())
				...
				if (!validSignalFound) {
					...
					if (ret) {
						if (!dataSourcesDatabase.Read("NumberOfElements",
								numberOfElements)) {
							numberOfElements = 1u;
							ret = dataSourcesDatabase.Write("NumberOfElements",
									numberOfElements);
						} else {
							...
							if (!ret) {
								REPORT_ERROR_PARAMETERS(
										ErrorManagement::InitialisationError,
										"NumberOfElements of signal %s in %s cannot be zero",
										signalName.Buffer(),
										dataSourceName.Buffer())
					...
					uint32 numberOfDimensions = 0u;
					if (ret) {
						if (!dataSourcesDatabase.Read("NumberOfDimensions",
								numberOfDimensions)) {
					...
					if (ret) {
						signalNumberOfBytes = (numberOfElements
								* signalTypeDescriptor.numberOfBits) / 8u;
					}
					if (ret) {
						ret = dataSourcesDatabase.Write("ByteSize",
								signalNumberOfBytes);
					}
					...
					if (ret) {
						...
						// check validity of the default value
							...
							if (ret) {
								...
								uint32 defValDims =
										defVal.GetNumberOfDimensions();
								ret = (defValDims <= 1u);
								uint8 usedDimensions =
										(numberOfDimensions > 0u) ? (1u) : (0u);
								if (ret) {
									ret = (defValDims == usedDimensions);
									if (ret) {
										ret = (defVal.GetNumberOfElements(0u)
												== numberOfElements);
									}
								} else {
									REPORT_ERROR_PARAMETERS(
								...
								if (ret) {
									void *ptr = HeapManager::Malloc(
											signalNumberOfBytes);
									ret = (ptr != NULL);
									if (ret) {
										AnyType at = AnyType(
												signalTypeDescriptor, 0u, ptr);

										at.SetNumberOfDimensions(
												usedDimensions);
										at.SetNumberOfElements(0u,
												numberOfElements);
								...
								if (!ret) {
									REPORT_ERROR_PARAMETERS(
											ErrorManagement::InitialisationError,
											"Unsupported defined default value of signal %s in %s",
											signalName.Buffer(),
											dataSourceName.Buffer())
								}
							}
		...
		for (i = 0u; (i < numberOfFunctions) && (ret); i++) {
			...
			if (exists) {
				...
				while ((s < numberOfSignals) && (ret)) {
					...
					if (ret) {
						//Expand the aliasName in order to write the correct name in the dataSourcesDatabase
						ret = functionsDatabase.Read("QualifiedName",
								signalName);
						if (ret) {
							if (!functionsDatabase.Read("Alias", aliasName)) {
					...
					StreamString dataSourceName;
					if (ret) {
						ret = functionsDatabase.Read("DataSource",
								dataSourceName);
					}
					if (!ret) {
						REPORT_ERROR_PARAMETERS(
								ErrorManagement::InitialisationError,
								"DataSource was not defined for signal: %s in %s",
					...
					bool deleted = false;
					if (ret) {
						ret = ResolveFunctionSignal(signalName.Buffer(),
								aliasName.Buffer(), functionName.Buffer(),
								dataSourceName.Buffer(), initNumberOfSignals,
								deleted);
					}
					if (ret) {
						if (deleted) {
...
bool RealTimeApplicationConfigurationBuilder::ResolveFunctionSignal(
		const char8 * const signalName, const char8 * const aliasName,
		const char8 * const functionName, const char8 * const dataSourceName,
		uint32 &numberOfFunctionSignals, bool &deleted) {

	deleted = false;
	uint32 numberOfSignalsInDataSource =
			dataSourcesDatabase.GetNumberOfChildren();
	StreamString type;
	...
	uint32 s;
	for (s = 0u;
			(s < numberOfSignalsInDataSource) && (ret)
					&& ((!typeDefined) || (!atLeastOneSignalFound)); s++) {
		...
		if (ret) {
			...
			char8 terminator;
			signalFound = true;
			while (aliasNameStr.GetToken(token1, ".", terminator)
					&& (signalFound)) {
		...
		if (signalFound) {
			if (!typeDefined) {
				if (createNestedSignal) {
					...
					if (ret) {
						...
						ret = newSignalId.Printf("%d",
								(numberOfFunctionSignals));
						if (ret) {
							ret = functionsDatabase.CreateRelative(
									newSignalId.Buffer());
						}
					}
					...
					if (ret) {
						...
						ret = functionsDatabase.Write("QualifiedName",
								newNodeName.Buffer());
					}
					if (ret) {
						ret = functionsDatabase.Write("DataSource",
								dataSourceName);
					}
				}
			}
			...
			const char8 *properties[] = { "Type", "NumberOfDimensions",
					"NumberOfElements", NULL_PTR(char8 *) };
			uint32 p = 0u;
			while ((properties[p] != NULL_PTR(char8 *)) && (ret)) {
				AnyType elementSignalDatabase = functionsDatabase.GetType(
						properties[p]);
				AnyType elementDataSourceDatabase = dataSourcesDatabase.GetType(
						properties[p]);
				if (elementSignalDatabase.GetTypeDescriptor() == VoidType) {
					if (elementDataSourceDatabase.GetTypeDescriptor()
							!= VoidType) {
						ret = functionsDatabase.Write(properties[p],
								elementDataSourceDatabase);
					} else {
						REPORT_ERROR_PARAMETERS(
								ErrorManagement::InitialisationError,
								"%s was not defined for signal: %s in %s",
								properties[p], aliasName, functionName)
					}
				}
				//Check for compatibility
					...
					if (!ret) {
						//Report mismatch!
						StreamString fullPropertyName;
						ret = fullPropertyName.Printf("%s.%s", aliasName,
								properties[p]);
						if (ret) {
							fullPropertyName = "Unknown";
						}
						REPORT_ERROR_PARAMETERS(
								ErrorManagement::InitialisationError,
								"Mismatch in signal with name: %s. %s asked for %s while %s asked for %s",
								fullPropertyName.Buffer(), functionName,
								sElementSignalDatabase.Buffer(), dataSourceName,
								sElementDataSourceDatabase.Buffer())
					}
				}
				p++;
			...
			if (ret) {
				...
				if (dataSourcesDatabase.Read("FullType", fullType)) {
					...
					if (ret) {
						ret = functionsDatabase.Write("FullType",
								fullType.Buffer());
					}
				...
				for (uint32 s = 0u; (s < numberOfSignals) && (ret); s++) {
					...
					if (ret) {
						...
						if (!ret) {
							REPORT_ERROR_PARAMETERS(
									ErrorManagement::InitialisationError,
									"Type was not defined for signal: %s in %s",
									signalName.Buffer(), functionName.Buffer())
						}
					}
					uint32 numberOfDimensions = 0u;
					if (ret) {
						ret = functionsDatabase.Read("NumberOfDimensions",
								numberOfDimensions);
						if (!ret) {
							REPORT_ERROR_PARAMETERS(
									ErrorManagement::InitialisationError,
									"NumberOfDimensions was not defined for signal: %s in %s",
									signalName.Buffer(), functionName.Buffer())
						}
					}
					uint32 numberOfElements = 0u;
					if (ret) {
						ret = functionsDatabase.Read("NumberOfElements",
								numberOfElements);
						if (!ret) {
							REPORT_ERROR_PARAMETERS(
									ErrorManagement::InitialisationError,
									"NumberOfElements was not defined for signal: %s in %s",
									signalName.Buffer(), functionName.Buffer())
						}
					}
					if (ret) {
...
bool RealTimeApplicationConfigurationBuilder::AddStateToGAM(
		const char8 * const gamName, const char8 * const stateName,
...
bool RealTimeApplicationConfigurationBuilder::AddStateToFunction(
		ConfigurationDatabase &local, const char8 * const functionName,
	...
	if (ret) {
		...
		if (numberOfGAMs == 0u) {
			...
			ret = AddStateToGAM(gamName.Buffer(), stateName, threadName,
					syncSignals);
		} else {
			uint32 i;
			for (i = 0u; (i < numberOfGAMs) && (ret); i++) {
			...
			if (globalDatabase.MoveRelative(stateName)) {
				...
				if (ret) {
					uint32 numberOfThreads =
							globalDatabase.GetNumberOfChildren();
					uint32 j;
					for (j = 0u; (j < numberOfThreads) && (ret); j++) {
						const char8 * threadName = globalDatabase.GetChildName(
								j);
							...
							if (ret) {
								...
								if (ret) {
									uint32 numberOfFunctions =
											at.GetNumberOfElements(0u);
									Vector<StreamString> functions(
											numberOfFunctions);
									ret = globalDatabase.Read("Functions",
											functions);
									uint32 syncSignals = 0u;
									uint32 k;
									for (k = 0u;
											(k < numberOfFunctions) && (ret);
											k++) {
										StreamString function = functions[k];
										ret = AddStateToFunction(local,
												function.Buffer(), stateName,
												threadName, syncSignals);
									}
									if (ret) {
										ret = (syncSignals <= 1u);
										if (!ret) {
											REPORT_ERROR_PARAMETERS(
													ErrorManagement::InitialisationError,
													"More than one synchronising signal found in %s.%s",
													&stateName[1],
													&threadName[1])
										}
									}
									...
									REPORT_ERROR_PARAMETERS(
											ErrorManagement::InitialisationError,
											"The Functions element must be defined as vector %s.%s",
											&stateName[1], &threadName[1])
								}
							} else {
								REPORT_ERROR_PARAMETERS(
										ErrorManagement::InitialisationError,
										"The Functions element must be %s.%s",
										&stateName[1], &threadName[1])
							}
							if (ret) {
...
bool RealTimeApplicationConfigurationBuilder::AddThreadCycleTime(
		const char8 * const threadFullName) {
	bool ret = dataSourcesDatabase.MoveAbsolute("Data");
	uint32 numberOfDataSources = dataSourcesDatabase.GetNumberOfChildren();
	...
	for (i = 0u; (i < numberOfDataSources) && (ret); i++) {
		...
		if (ret) {
			uint32 isTimingDataSource;
			if (dataSourcesDatabase.Read("IsTimingDataSource",
					isTimingDataSource)) {
				if (isTimingDataSource == 1u) {
					ret = dataSourcesDatabase.MoveRelative("Signals");
					if (ret) {
						uint32 numberOfTimeSignals =
								dataSourcesDatabase.GetNumberOfChildren();
						bool found = false;
						uint32 j;
						for (j = 0u;
								(j < numberOfTimeSignals) && (ret) && (!found);
								j++) {
							StreamString signalId;
						...
						if (ret && (!found)) {
							...
							if (ret) {
								ret = dataSourcesDatabase.CreateRelative(
										newSignalId.Buffer());
							}
							if (ret) {
...
bool RealTimeApplicationConfigurationBuilder::WriteTimeSignalInfo(
	...
	if (ret) {
		uint32 numberOfDimensions = 0u;
		ret = dataSourcesDatabase.Write("NumberOfDimensions",
				numberOfDimensions);
	}
	if (ret) {
		uint32 numberOfElements = 1u;
	...
	for (i = 0u; (i < numberOfFunctions) && (ret); i++) {
		...
		if (ret) {
			...
			if (!ret) {
				...
				REPORT_ERROR_PARAMETERS(ErrorManagement::FatalError,
						"The GAM %s is never called", gamName.Buffer())
			}
			...
			if ((functionsDatabase.MoveRelative("States")) && (ret)) {
				...
				if (numberOfStates > 0u) {
					...
					for (s = 0u; (s < numberOfStates) && (ret); s++) {
						StreamString stateName = functionsDatabase.GetChildName(
								s);
						states[s] = stateName;
					}
					if (ret) {
					...
					bool exists = false;
					if (ret) {
						exists = functionsDatabase.MoveRelative(
								signalDirection);
					}
					if (exists) {
						uint32 numberOfSignals =
						...
						for (i = 0u; (i < numberOfSignals) && (ret); i++) {
							...
							ret = functionsDatabase.MoveRelative(
									signalId.Buffer());
							...
							if (ret) {
								...
								ret = functionsDatabase.Read("QualifiedName",
										signalName);
								if (ret) {
									if (functionsDatabase.Read("Alias",
											alias)) {
										signalName = alias;
									}
								}
								...
								if (ret) {
									ret = dataSourcesDatabase.MoveAbsolute(
											"Data");
								}
								if (ret) {
									ret = dataSourcesDatabase.MoveRelative(
								...
								if (ret) {
									...
									ret = FindSignalName(signalName,
											dataSourcesDatabase);
								}
								if (ret) {
									if (!dataSourcesDatabase.MoveRelative(
											"States")) {
										ret =
												dataSourcesDatabase.CreateRelative(
														"States");
									}
								}

								//For all the states
								for (s = 0u; (s < numberOfStates) && (ret);
										s++) {
									if (!dataSourcesDatabase.MoveRelative(
											states[s].Buffer())) {
										ret =
									...
									if (ret) {
										...
										StreamString operationTypeGAMNames =
												"GAMNames";
										operationTypeGAMNames += operationType;
										StreamString operationTypeSignal =
												"Signal";
										operationTypeSignal += operationType;
										AnyType existentArray =
												dataSourcesDatabase.GetType(
														operationTypeGAM.Buffer());
										StreamString *newGAMArray = NULL_PTR(
												StreamString *);
										StreamString *newGAMNamesArray =
												NULL_PTR(StreamString *);
										StreamString *newSignalArray = NULL_PTR(
												StreamString *);

										uint32 numberOfExistentElements = 0u;
										if (existentArray.GetDataPointer() != NULL_PTR(void *)) {
											numberOfExistentElements =
													existentArray.GetNumberOfElements(
															0u);
											const uint32 numberOfNewsElements =
													(numberOfExistentElements
															+ 1u);
											newGAMArray =
													new StreamString[numberOfNewsElements];
											newGAMNamesArray =
													new StreamString[numberOfNewsElements];
											newSignalArray =
													new StreamString[numberOfNewsElements];
											Vector<StreamString> newGAMVector(
													newGAMArray,
													numberOfExistentElements);
											Vector<StreamString> newGAMNamesVector(
													newGAMNamesArray,
													numberOfExistentElements);
											Vector<StreamString> newSignalVector(
													newSignalArray,
													numberOfExistentElements);
											ret = (dataSourcesDatabase.Read(
													operationTypeGAM.Buffer(),
													newGAMVector));
											if (ret) {
												ret =
														(dataSourcesDatabase.Read(
																operationTypeGAMNames.Buffer(),
																newGAMNamesVector));
											}
											if (ret) {
												ret =
														(dataSourcesDatabase.Read(
																operationTypeSignal.Buffer(),
																newSignalVector));
											}
										} else {
										...
										newGAMArray[numberOfExistentElements] =
												functionId.Buffer();
										newGAMNamesArray[numberOfExistentElements] =
												functionName.Buffer();
										newSignalArray[numberOfExistentElements] =
												signalId.Buffer();
										Vector<StreamString> newGAMVector(
												newGAMArray,
												numberOfExistentElements + 1u);
										Vector<StreamString> newGAMNamesVector(
												newGAMNamesArray,
												numberOfExistentElements + 1u);
										Vector<StreamString> newSignalVector(
										...
										if (existentArray.GetDataPointer() != NULL_PTR(void *)) {
											ret = dataSourcesDatabase.Delete(
													operationTypeGAM.Buffer());
											if (ret) {
												ret =
														dataSourcesDatabase.Delete(
									...
									if (ret) {
										ret =
												dataSourcesDatabase.MoveToAncestor(
														1u);
									}
								}
								//Move to the States level
								if (ret) {
									ret = dataSourcesDatabase.MoveToAncestor(
											1u);
								}
							}
							//Move to the next signal
				...
				if (ret) {
					...
					for (uint32 k = 0u; (k < numberOfStates) && (ret); k++) {
						...
						if (ret) {
							...
							AnyType prods = dataSourcesDatabase.GetType(
									"GAMProducers");
							if (isTimeDs) {
								...
								if (!ret) {
									REPORT_ERROR_PARAMETERS(
											...
											"The time signal %s in %s cannot be produced",
											dsSignalName.Buffer(),
											dataSourceName.Buffer())
								}
							} else {
		...
		if (ret) {
			...
			if (ret) {
				...
				if (ret) {
					...
					Matrix<uint32> rangesMatrix = Matrix<uint32>(rangeRows,
							rangeCols);
					...
					for (uint32 k = 0u; (k < rangeRows) && (ret); k++) {
						...
						if (reachSignalRanges >= (minNumberOfSignalRanges)) {
							...
							uint32 copySize = minNumberOfSignalRanges
									* static_cast<uint32>(sizeof(uint32));
							ret = MemoryOperationsHelper::Copy(temp,
									signalRanges, copySize);
							minNumberOfSignalRanges = newSize;
							delete[] signalRanges;
							signalRanges = temp;
						}
						if (ret) {
							signalRanges[reachSignalRanges] = rangesMatrix(k,
									0u);
							reachSignalRanges++;
							signalRanges[reachSignalRanges] = rangesMatrix(k,
									1u);
...
bool RealTimeApplicationConfigurationBuilder::CheckProducersRanges(
		const uint32 * const rangesArray, const uint32 numberOfElements) const {
	bool ret = ((numberOfElements % 2u) == 0u);
	for (uint32 i = 1u; (i < numberOfElements) && ret; i += 2u) {
		for (uint32 j = i + 2u; (j < numberOfElements) && (ret); j += 2u) {
			bool isFirstMin = (rangesArray[i - 1u] < rangesArray[j - 1u])
					&& (rangesArray[i] < rangesArray[j - 1u]);
			bool isFirstMax = (rangesArray[j - 1u] < rangesArray[i - 1u])
					&& (rangesArray[j] < rangesArray[i - 1u]);
...
bool RealTimeApplicationConfigurationBuilder::AddTimingSignals() {
	...
	if (ret) {
		...
		for (i = 0u; (i < numberOfFunctions) && (ret); i++) {
			...
			const char8 * postfix[] = { "_ReadTime", "_ExecTime", "_WriteTime",
				...
				for (uint32 s = 0u; (s < numberOfSignals) && (ret); s++) {
					...
					if (ret) {
						...
						if (ranges.GetTypeDescriptor() != VoidType) {
							uint32 numberOfDimensions =
									ranges.GetNumberOfDimensions();
							ret = (numberOfDimensions == 2u);
							if (ret) {
								numberOfRanges = ranges.GetNumberOfElements(1u);
								if (numberOfRanges > 0u) {
									const uint32 backendRangesSize =
											numberOfRanges * 2u;
									uint32 *rangesMatBackend =
											new uint32[backendRangesSize];
									offsetMatrixBackend =
											new uint32[backendRangesSize];
									//The offset matrix stores, for each range and in bytes, the starting offset and the size of the range to copy.
									Matrix<uint32> offsetMat(
											offsetMatrixBackend, numberOfRanges,
											2u);
									//Read the Ranges matrix from the configuration data.
									Matrix<uint32> rangesMat(rangesMatBackend,
											numberOfRanges, 2u);
									ret = functionsDatabase.Read("Ranges",
											rangesMat);
									if (ret) {
										for (uint32 n = 0u;
												(n < numberOfRanges) && (ret);
												n++) {
											...
											if (ret) {
												uint32 rangeByteSize =
														(((maxIdx - minIdx) + 1u)
																* signalTypeDescriptor.numberOfBits)
																/ 8u;
												signalNumberOfBytes +=
														rangeByteSize;
												offsetMat(n, 0u) =
					...
					if (ret) {
						ret = functionsDatabase.Write("ByteSize",
								signalNumberOfBytes);
					}
					if (ret) {
						if (offsetMatrixBackend != NULL_PTR(uint32 *)) {
							Matrix<uint32> offsetMat(offsetMatrixBackend,
									numberOfRanges, 2u);
							ret = functionsDatabase.Write("ByteOffset",
									offsetMat);
						}
					}
					if (offsetMatrixBackend != NULL_PTR(uint32 *)) {
		...
		for (i = 0u; (i < numberOfFunctions) && (ret); i++) {
			...
			if (exists) {
				...
				for (s = 0u; (s < numberOfSignals) && (ret); s++) {
					...
					if (ret) {
						AnyType existentByteOffset = functionsDatabase.GetType(
								"ByteOffset");
						if (existentByteOffset.GetDataPointer() != NULL_PTR(void *)) {
							numberOfOffsetElements =
									existentByteOffset.GetNumberOfElements(1u);
							// create a static matrix
							const uint32 offsetMatrixBackendNElements =
									numberOfOffsetElements * 2u;
							offsetMatrixBackend =
									new uint32[offsetMatrixBackendNElements];
							Matrix<uint32> offsetMat(offsetMatrixBackend,
									numberOfOffsetElements, 2u);
							ret = functionsDatabase.Read("ByteOffset",
									offsetMat);
						} else {
					...
					if (ret) {
						if (!functionsDatabase.Read("Samples",
								samplesBackend)) {
							samplesBackend = 1u;
						} else {
							...
							if (!ret) {
								REPORT_ERROR_PARAMETERS(
										ErrorManagement::InitialisationError,
										"Invalid Samples for signal %s in %s",
										signalName.Buffer(),
										functionName.Buffer())
							}
						}
					...
					if (ret) {
						...
						if (!functionsDatabase.Read("Frequency",
								frequencyBackend)) {
					...
					if (ret) {
						...
						uint32 numberOfDataSources =
								functionsDatabase.GetNumberOfChildren();
						bool found = false;
						StreamString dataSourceId;
						for (uint32 d = 0u;
								(d < numberOfDataSources) && (ret) && (!found);
								d++) {
							...
							if (ret) {
								found = (StringHelper::Compare(
										thisDataSourceName.Buffer(),
										dataSourceName.Buffer()) == 0);
							}
							//Move to the next DataSource
							if (!found) {
						...
						if (ret) {
							if (!found) {
								...
								ret = newDataSourceId.Printf("%d",
										numberOfDataSources);
								if (ret) {
									ret = functionsDatabase.CreateRelative(
											newDataSourceId.Buffer());
								}
								if (ret) {
									ret = functionsDatabase.Write("DataSource",
											dataSourceName.Buffer());
					...
					if (ret) {
						if (functionsDatabase.Read("ByteSize",
								existentByteSize)) {
					...
					if (ret) {
						ret = functionsDatabase.Write("Samples",
								samplesBackend);
					}

					if (ret) {
						ret = functionsDatabase.Write("Frequency",
								frequencyBackend);
					}

					if (ret) {
						ret = functionsDatabase.Write("GAMMemoryOffset",
								allocatedByteSize);
					}
		...
		for (i = 0u; (i < numberOfFunctions) && (ret); i++) {
			...
			if (exists) {
				...
				for (d = 0u; (d < numberOfDataSources) && (ret); d++) {
					...
					StreamString dataSourceName;
					StreamString dataSourceId = functionsDatabase.GetChildName(
							d);
					...
					if (ret) {
						ret = FindDataSourceNumber(dataSourceName.Buffer(),
								dataSourceIdInDataSourceDatabase);
					}
					if (ret) {
						ret = dataSourcesDatabase.MoveRelative(
								dataSourceIdInDataSourceDatabase.Buffer());
					}
					...
					if (ret) {
						...
						uint32 n;
						uint32 numberOfDataSourceFunctions =
								dataSourcesDatabase.GetNumberOfChildren();
						StreamString functionId;
						bool found = false;
						for (n = 0u;
								(n < numberOfDataSourceFunctions) && (ret)
										&& (!found); n++) {
							...
							if (ret) {
								found = (StringHelper::Compare(
										thisFunctionName.Buffer(),
										functionName.Buffer()) == 0);
							}
							//Move to the next DataSource
							if (!found) {
								ret = dataSourcesDatabase.MoveToAncestor(1u);
						...
						if (ret) {
							if (!found) {
								...
								ret = newFunctionId.Printf("%d",
										numberOfDataSourceFunctions);
								if (ret) {
									ret = dataSourcesDatabase.CreateRelative(
											newFunctionId.Buffer());
								}
								if (ret) {
									ret = dataSourcesDatabase.Write(
											"QualifiedName",
					...
					if (ret) {
						uint32 numberOfSignalsInFunction =
								functionsDatabase.GetNumberOfChildren();
						uint32 numberOfSignalsAlreadyInDataSource =
								dataSourcesDatabase.GetNumberOfChildren();
						uint32 n;
						for (n = 0u; (n < numberOfSignalsInFunction) && (ret);
								n++) {
							StreamString functionSignalId =
									functionsDatabase.GetChildName(n);
							ret = functionsDatabase.MoveRelative(
									functionSignalId.Buffer());
							StreamString newSignalId;
							if (ret) {
								ret =
										newSignalId.Printf("%d",
												(n
														+ numberOfSignalsAlreadyInDataSource));
								if (ret) {
									ret = dataSourcesDatabase.CreateRelative(
											newSignalId.Buffer());
								}
...
bool RealTimeApplicationConfigurationBuilder::AssignBrokersToSignals(
		const SignalDirection direction, ReferenceT<DataSourceI> dataSource) {
	...
	if (dataSourcesDatabase.MoveRelative(dirStr)) {
		...
		for (i = 0u; (i < numberOfSignals) && (ret); i++) {
			...
			if (dataSourcesDatabase.MoveRelative(signalId.Buffer())) {
				...
				StreamString brokerClassName = dataSource->GetBrokerName(
						dataSourcesDatabase, direction);
				ret = brokerClassName.Size() > 0u;
				if (ret) {
					ret = dataSourcesDatabase.Write("Broker",
							brokerClassName.Buffer());
				} else {
					StreamString signalName;
					if (!dataSourcesDatabase.Read("QualifiedName",
							signalName)) {
						signalName = "UnknownSignal";
					}
					REPORT_ERROR_PARAMETERS(
							ErrorManagement::InitialisationError,
							"Unsupported broker for signal %s linked to %s",
		...
		for (n = 0u; (n < numberOfFunctions) && (ret); n++) {
			...
			if (ret) {
				...
				if (!ret) {
					REPORT_ERROR_PARAMETERS(
							ErrorManagement::InitialisationError,
							"The GAM %s is not valid", qualifiedName.Buffer())
				}
			}
			if (ret) {
...
bool RealTimeApplicationConfigurationBuilder::Copy(
		ConfigurationDatabase &functionsDatabaseOut,
...
bool RealTimeApplicationConfigurationBuilder::Set(
...
bool RealTimeApplicationConfigurationBuilder::FindSignalName(
		StreamString signalName, ConfigurationDatabase &database) const {
	bool ret = true;
	bool found = false;
	...
	for (j = 0u; (j < numberOfSignalsInDatabase) && (ret) && (!found); j++) {
		...
		if (ret) {
			...
			if (ret) {
				found = (StringHelper::Compare(signalName.Buffer(),
						databaseSignalName.Buffer()) == 0);
			}
		}
		if (ret) {
			//Move to the next signal
...
bool RealTimeApplicationConfigurationBuilder::FindFunctionNumber(
		StreamString functionName, StreamString &functionNumber) {
	bool ret = functionsDatabase.MoveAbsolute("Functions");
	if (ret) {
		uint32 numberOfFunctions = functionsDatabase.GetNumberOfChildren();
		uint32 n;
...
bool RealTimeApplicationConfigurationBuilder::CheckTypeCompatibility(
		StreamString &fullType, StreamString &otherFullType,
...
bool RealTimeApplicationConfigurationBuilder::SignalIntrospectionToStructuredData(
		ConfigurationDatabase &signalDatabase, const char8 * const typeName,
		const char8 * const signalName, const char8 * const alias,
		const char8 * const dataSourceName, const char8 * const syncSignalName,
		const char8 * const fullTypeName, const AnyType & ranges,
		const AnyType & samples, const AnyType & frequency,
		StructuredDataI & data, uint32 &signalNumber, bool &syncSet,
		const bool isFunctionDatabase) {
	//Try to find the registered type in the ClassRegistryDatabase
	const ClassRegistryItem *item = ClassRegistryDatabase::Instance()->Find(
			typeName);
	...
	if (ret) {
		...
		REPORT_ERROR_PARAMETERS(ErrorManagement::FatalError,
				"The type %s is not registered", typeName)
	}
	...
	if (ret) {
		...
		for (i = 0u; (i < numberOfMembers) && (ret); i++) {
			...
			ret = fullSignalName.Printf("%s.%s", signalName,
					entry.GetMemberName());
			...
			if (StringHelper::Length(alias) > 0u) {
				...
				if (ret) {
					ret = fullAliasName.Printf("%s.%s", alias,
							entry.GetMemberName());
				}
			}
			...
			if (isStructured) {
				ret = SignalIntrospectionToStructuredData(signalDatabase,
						entry.GetMemberTypeName(), fullSignalName.Buffer(),
						fullAliasName.Buffer(), dataSourceName, syncSignalName,
						typeNameStr.Buffer(), ranges, samples, frequency, data,
						signalNumber, syncSet, isFunctionDatabase);
			} else {
				if (ret) {
				...
				if (signalDatabase.MoveRelative("Defaults")) {
					...
					if (signalDatabase.Read(fullSignalName.Buffer(),
							defaultValue)) {
				...
				if (ret) {
					ret = data.Write("NumberOfDimensions",
							entry.GetNumberOfDimensions());
				}
				if (ret) {
						...
						if (ret) {
							if (frequency.GetTypeDescriptor() != VoidType) {
								if (StringHelper::Compare(
										fullSignalName.Buffer(), syncSignalName)
										== 0) {
...
bool RealTimeApplicationConfigurationBuilder::SearchGAMs(
		ConfigurationDatabase &inputDatabase,
		ConfigurationDatabase &outputDatabase, StreamString & fullPath,
		uint32 &index, bool found) {
	...
	if (isGam) {
		...
		if (!isGam) {
			...
			if (!isGam) {
				ret = false;
				REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError,
						"Specified GAM %s with no input nor output",
						fullPath.Buffer())
			}
		}
	}
	...
	if (isGam) {
		...
		if (found) {
			ret = false;
			REPORT_ERROR_PARAMETERS(ErrorManagement::InitialisationError,
					"Unsupported nested GAMs in path %s", fullPath.Buffer())
		} else {
			found = true;
		}
	...
	for (uint32 i = 0u; (i < numberOfFunctions) && (ret); i++) {
		...
		if (inputDatabase.MoveRelative(nodeName)) {
			...
			ret = SearchGAMs(inputDatabase, outputDatabase, currentStep, index,
					found);
...
uint32 RealTimeApplicationConfigurationBuilder::GetNumberOfSyncSignals(
		const char8 * const direction, bool &ok) {
...
bool RealTimeApplicationConfigurationBuilder::SearchDataSources(
		ConfigurationDatabase &inputDatabase,
		ConfigurationDatabase &outputDatabase, StreamString & fullPath,
		uint32 &index, uint32 &timingDataSourceCounter) {
	bool ret = true;
	...
	if (isDataSource) {
		...
		if (ret) {
			if (className == "TimingDataSource") {
				uint32 isTimingDataSource;
				if (!outputDatabase.Read("IsTimingDataSource",
						isTimingDataSource)) {
					isTimingDataSource = 1u;
					ret = outputDatabase.Write("IsTimingDataSource",
							isTimingDataSource);
				}
	...
	for (uint32 i = 0u; (i < numberOfFunctions) && (ret); i++) {
		...
		if (inputDatabase.MoveRelative(nodeName)) {
			...
			ret = SearchDataSources(inputDatabase, outputDatabase, currentStep,
					index, timingDataSourceCounter);
