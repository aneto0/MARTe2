...
#define REGISTER_PCODE_FUNCTION(name,subName,nInputs,nOutputs,function,...)\
    /*lint --emacro(446) Justification: Type2TypeDescriptor<>() is known to have no side-effects and can thus be used safely in ..._FunctionTypes[] array initialization.*/ \
    ...
    /*lint --emacro{1502} Justification: the following class intentionally has no data member. */ \
