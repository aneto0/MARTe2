...
static bool StringToIntegerDecimalNotation(const char8* const input,
    ...
    for (; (!canReturn) && (i < 1000u); i++) {
        ...
        if ((newDigit >= 0) && (newDigit <= 9)) {
            ...
            // number > max
            if (number > max) {
                REPORT_ERROR_STATIC(ErrorManagement::Overflow, "StringToIntegerDecimalNotation: Overflow.");
                canReturn = true;
                ret = false;
            }
            if (number == max) {
            ...
            if (digit != '\0') {
                ret = false;
                REPORT_ERROR_STATIC(ErrorManagement::SyntaxError, "StringToIntegerDecimalNotation: Invalid token.");
            }
            canReturn = true;
        }
    }
    ...
    if (isNegative) {
        if (isSigned) {
            ...
            number = static_cast<T>(0);
            ret = false;
            REPORT_ERROR_STATIC(ErrorManagement::OutOfRange, "StringToIntegerDecimalNotation: The string represents a negative number and output type is unsigned.");
        }
    }
    ...
    for (; (!canReturn) && (i < 1000u); i++) {
        ...
        if ((newDigit < 0) || (newDigit > 9)) {
            ...
            if ((newDigit < 10) || (newDigit > 15)) {
                ...
                if ((newDigit < 10) || (newDigit > 15)) {
                    if (digit != '\0') {
                        ret = false;
                        REPORT_ERROR_STATIC(ErrorManagement::SyntaxError, "StringToIntegerExadecimalNotation: Invalid token.");
                    }
                    canReturn = true;
                }
            }
        ...
        if (!canReturn) {
            ...
            if (totalShift > (static_cast<uint32>(sizeof(T) * 8u))) {
                REPORT_ERROR_STATIC(ErrorManagement::Overflow, "StringToIntegerExadecimalNotation: Overflow.");
                ret = false;
                canReturn = true;
            }
            else {
        ...
        if ((newDigit < 0) || (newDigit > 7)) {
            ...
            if (totalShift > static_cast<uint32>(sizeof(T) * 8u)) {
                if (totalShift == static_cast<uint32>((sizeof(T) * 8u) + 1u)) {
                    if (firstDigit > 3) {
                        REPORT_ERROR_STATIC(ErrorManagement::Overflow, "StringToIntegerOctalNotation: Overflow.");
                        canReturn = true;
                        ret = false;
                    }
                }
...
static bool StringToIntegerBinaryNotation(const char8* const input,
    ...
    for (; (!canReturn) && (i < 1000u); i++) {
        ...
        if ((newDigit < 0) || (newDigit > 1)) {
            ...
            totalShift++;
            if (totalShift > static_cast<uint32>(sizeof(T) * 8u)) {
                REPORT_ERROR_STATIC(ErrorManagement::Overflow, "StringToIntegerBinaryNotation: Overflow.");
                canReturn = true;
                ret = false;
            }
            else {
...
static bool StringToInteger(const char8 * const input,
    ...
    if (input[0] == '0') {
        ...
        case ('\0'): {
            ...
            break;
        default: {
            REPORT_ERROR_STATIC(ErrorManagement::SyntaxError, "StringToIntegerPrivate: Invalid token.");
            ret = false;
        }
        }
    }
