...
static void PrintTypeDescriptor(DynamicCString &line,const TypeDescriptor &td){
    ...
    if (td.isStructuredData){
    ...
    } else {// not structuredData
    	if (td.IsComplexType()){  //Stream,StructuredData,..
        	line.AppendN(constString);
        	line.AppendN(BasicTypeName(td.type,td.complexType));
    	} else  // not complexType
        ...
        // this section should never execute as AnyType does not supports this
        // it would use instead the modifiers
        // only generated when browsing a variable from the top
        // to be removed and replaced with an error
        // TODO
        ...
        { // not static zero term array
            ...
            case ConstDynamicZeroTermArray:
            	...
            	if ((td.type == Char) && (!td.dataIsConstant)) {
                	line.AppendN("CString");
            	} else
                if ((td.type == Char) && (td.dataIsConstant)) {
                   	line.AppendN("CCString");
                } else {
            		// TODO
            		// generic type
            		// not generated by AnyType/VariableDescription
            		// it uses modifiers instead
...
static void PrintVariableDescriptor(DynamicCString &line,const TypeDescriptor &td){
