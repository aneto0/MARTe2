...
static uint16 GetOrderOfMagnitude(const T positiveNumber) {
    ...
    if (sizeof(T) >= 8u) { // max 19
        /*lint -e{734} */
        T temp = static_cast<T>(tenToExponent * 10000000000u); // 10 zeros
    ...
    if (sizeof(T) >= 4u) { // max 9
        /*lint -e{734} */
        T temp = static_cast<T>(tenToExponent * 100000u); // 5 zeros
        if (positiveNumber >= temp) {
            tenToExponent = temp;
            nDigits += 5u;
        }
    ...
    if (sizeof(T) >= 2u) { // max 4 zeros
        /*lint -e{734} */
        T temp = static_cast<T>(tenToExponent * 100u); // 2 zeros
        if (positiveNumber >= temp) {
            tenToExponent = temp;
            nDigits += 2u;
        }
    ...
    /*lint -e{734} */
    T temp = static_cast<T>(tenToExponent * 10u); // 1
    if (positiveNumber >= temp) {
        tenToExponent = temp;
        nDigits++;
    }
    ...
    // check whether exponent greater than 1
    /*lint -e{734} */
    temp = static_cast<T>(tenToExponent * 10u);  // 1
...
bool BitSetToStream(IOBuffer &ioBuffer,
                    uint32 const * const numberAddress,
