...
bool Move(void * const destination,
    ...
    if ((source != NULL) && (destination != NULL)) {
        ...
        /* WARNING - DO NOT DIRECTLY USE MEMMOVE, MISBEHAVIOUR DETECTED ON OVERLAPPING MOVE */
        //memmove from circle-stdlib has a big issue when dealing with overlapping memory

        void* tempData = (void*)malloc(size);
        ret = tempData != NULL;

        if(ret) {
            ret = memcpy(tempData, source, size) != NULL;    
        }
        
        if(ret) {
            ret = memcpy(destination, tempData, size) != NULL;        
        }
