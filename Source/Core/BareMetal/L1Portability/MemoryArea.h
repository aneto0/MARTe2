...
public:
     ...
     * MemoryArea::Add(*) and MemoryArea::Free(*) can deallocate \a initialMemory, then do not use
     * to use the memory pointed by \a initialMemory outside MemoryArea.
     * @return true if the memory area is empty, false if it is already initialised.
     */
    bool InitMemory(void * initialMemory, uint32 initialSize);
