 ...
 *
 * @warning The instances of AnyType do not hold a copy of the actual value,
 * but a pointer to it, so the value must be accessible during the life of the
 * AnyType instance. Pay special attention to temporary created values like in
 * the following example:
 *
 * AnyType list[] = { 2, 5, 8, 9 };
 *
 * This will fail with the code generated by some compilers, while it will be
 * ok with others. When it goes wrong, all the AnyType instances will point to
 * the same address, i.e. the same value.
 */
