...
/*                           Method definitions                              */
/*---------------------------------------------------------------------------*/
ReferenceContainerFilterObjects::ReferenceContainerFilterObjects():ReferenceContainerFilter(){
...
bool ReferenceContainerFilterObjects::Test(ReferenceContainer &previouslyFound,
    ...
    if (referenceToTest.IsValid()) {
        if (referenceToTest->GetName() != NULL) {
            if (addressNumberNodes > 0u) {        /////////////////////////////  BUG HERE!!!
    ...
    if (found) {
        if ((addressNumberNodes == 1u) || (addressNumberNodes == (previouslyFound.Size() + 1u))) {
            IncrementFound();
        }
    }
    return found;
