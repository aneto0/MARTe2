...
ThreadInformation::ThreadInformation(const ThreadFunctionType threadFunction,
    ...
    if (threadName != NULL) {
        ...
        if (this->name == NULL) {
            REPORT_ERROR_STATIC_0(ErrorManagement::FatalError, "Error: duplication of thread name failed");
        }
    }
    else {
        this->name = StringHelper::StringDup("Unknown");
        if (this->name == NULL) {
            REPORT_ERROR_STATIC_0(ErrorManagement::FatalError, "Error: duplication of thread name \"Unknown\" failed");
        }
    }
    threadId = InvalidThreadIdentifier;
    priorityClass = Threads::UnknownPriorityClass;
