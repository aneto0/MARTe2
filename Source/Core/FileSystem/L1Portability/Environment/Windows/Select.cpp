...
Select::Select() {
    ...
    if (err != 0) {
        REPORT_ERROR_STATIC_0(ErrorManagement::FatalError, "Select::Select(), Fail WSAStartup.");
    }
...
bool Select::AddReadHandle(const HandleI &handle) {
    ...
    if (reinterpret_cast<int32>(h) >= 0) {
        if (retVal) {
            ...
            for (int32 i = 0; i < indexMax; i++) {
                if (readHandle.registeredHandles[i] == h) {
                    REPORT_ERROR_STATIC_0(ErrorManagement::FatalError, "Select::AddHandle(), Same Handle.");
                    retVal = false;
                }
            }
        }
        ...
        REPORT_ERROR_STATIC_0(ErrorManagement::FatalError, "Select::AddHandle(). Invalid descriptor.");
        retVal = false;
    }
    return retVal;
}
