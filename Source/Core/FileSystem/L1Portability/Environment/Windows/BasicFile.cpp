...
static MARTe::uint32 CheckFlags(MARTe::uint32 flags)
{
    ...
    if ((flags & (MARTe::BasicFile::FLAG_CREAT | MARTe::BasicFile::FLAG_CREAT_EXCLUSIVE)) == (MARTe::BasicFile::FLAG_CREAT | MARTe::BasicFile::FLAG_CREAT_EXCLUSIVE))
    {
    ...
    if ((flags & (MARTe::BasicFile::FLAG_TRUNC | MARTe::BasicFile::FLAG_CREAT_EXCLUSIVE)) == (MARTe::BasicFile::FLAG_TRUNC | MARTe::BasicFile::FLAG_CREAT_EXCLUSIVE))
    {
        MARTe::uint32 clean_TRUNC = ~MARTe::BasicFile::FLAG_TRUNC;
        flags &= clean_TRUNC;
    }
...
namespace MARTe
{
                         ...
                         const TimeoutType &msecTimeout)
    {
        ...
        if (IsOpen() && CanRead())
        {
            ...
            {
                bool readVal;
                readVal = ReadFile(properties.handle, (void *)output, size, &actual, &ol);
                if (!readVal)
                {
                    if (GetLastError() == ERROR_IO_PENDING)
                    {
                        // REPORT_ERROR_STATIC_0(ErrorManagement::OSError, "IO PEnd");
                        do
                        {
                            ok = GetOverlappedResult(properties.handle, &ol, &actual, true);
                            if (ok)
                            {
         //                       printf("\nPend-End Read %d Offset %d\n", actual, oldpos);
                                size = actual;
                            }
                            else
                            {
