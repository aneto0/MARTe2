...
.. warning::
    ...
    The EPICSPVAInput DataSource is only available in distributions where `EPICS Channel Access <https://epics-controls.org/>`__ is installed.

The :vcisdoxygenmccl:`EPICSPVAInput` DataSource can be used to receive application data using the EPICS pvAccess protocol. This allows interfacing the application with any EPICS-based monitoring or control application (e.g. `EPICS Control System Studio <https://controlsystemstudio.org/>`__).
