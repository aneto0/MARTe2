...
The ``TriggeredIOGAM`` is commonly used to write to an output DataSource (e.g. logger, file, ...) only when a trigger condition is met. In the mass-spring-damper example, the ``TriggeredIOGAM`` is used to log the system state only until the position converges to its final position.
...
.. dropdown:: Solution
   ...
   The solution is to add to modify the ``GAMMathTrigger`` to implement the time based expression.
