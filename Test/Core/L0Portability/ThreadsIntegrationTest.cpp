/**
 * @file ThreadsIntegrationTest.cpp
 * @brief Source file for class ThreadsIntegrationTest
 * @date 25/06/2015
 ...
 * the class ThreadsIntegrationTest (public, protected, and private). Be aware that some 
...
ThreadsIntegrationTest::ThreadsIntegrationTest() {
...
void ThreadsTestIncrementCounter(ThreadsIntegrationTest &tt) {
    tt.incrementCounter++;
}

void ThreadsTestWaitIncrementCounter(ThreadsIntegrationTest &tt) {
...
void PrioritiesCallback(ThreadsIntegrationTest &tt) {
    bool ok = True;
...
bool ThreadsIntegrationTest::BeginThread(uint32 nOfThreads) {
    uint32 i = 0;
...
bool ThreadsIntegrationTest::Priorities() {
    eventSem.Reset();
...
void PriorityClassRunFirst(ThreadsIntegrationTest &tt) {
...
bool ThreadsIntegrationTest::PrioritiesPropagationStartFirst() {
    int32 k = 0; //count the number of threads created.
    incrementCounter = 0;
    nOfThreads1 = 28; //7 levels x 4 classes
    eventSem.Reset();
    while (k < (int32) nOfThreads1) {
...
void CheckIdDifferentTo0(ThreadsIntegrationTest &tt) {
    TID threadId = 0;
...
bool ThreadsIntegrationTest::ThreadIdTest() {
    incrementCounter = 0;
    ...
    callbackTestSuccessful = callbackTestSuccessful && (nOfThreads1 == incrementCounter);
    return callbackTestSuccessful;
}

void AutoKillThread(ThreadsIntegrationTest &tt) {
...
void InfiniteLoop(ThreadsIntegrationTest &tt) {
...
void ThreadManagement(ThreadsIntegrationTest &tt) {
    tt.falseId = (TID) - 1;
    bool ok = false;
...
bool ThreadsIntegrationTest::ThreadKillTest() {
    eventSem.Reset();
...
bool ThreadsIntegrationTest::ThreadNameTest() {
    falseId = (TID) - 1;
    eventSem.Reset();
    ...
    callbackTestSuccessful = StringTestHelper::Compare(ThreadsName(threadIdTarget), nameThreadTest);
...
void EndTarget(ThreadsIntegrationTest &tt) {
    tt.threadIdTarget = Threads::Id();
    //Main can continue
    tt.eventSem.Post();
...
void ThreadManagementEnd(ThreadsIntegrationTest &tt) {
    bool ok;
...
bool ThreadsIntegrationTest::ThreadEndTest() {
    eventSem.Reset();
