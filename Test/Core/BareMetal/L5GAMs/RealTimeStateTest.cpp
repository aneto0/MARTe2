...
bool RealTimeStateTestGAMGroup1::PrepareNextState(
		const char8 * currentStateName, const char8 * nextStateName) {

	context = ReferenceT<RealTimeStateTestContext1>(
			"RealTimeStateTestContext1");
...
bool RealTimeStateTestGAMGroup2::PrepareNextState(
		const char8 * currentStateName, const char8 * nextStateName) {
...
bool RealTimeStateTest::TestAddStatefuls() {
	ReferenceT<RealTimeState> state1 = ObjectRegistryDatabase::Instance()->Find(
			"Fibonacci.States.State1");
	if (!state1.IsValid()) {
		return false;
	}
	ReferenceContainer statefuls;
	ReferenceT<ReferenceContainer> functions =
			ObjectRegistryDatabase::Instance()->Find("Fibonacci.Functions");
	...
	ReferenceContainerFilterReferencesTemplate<StatefulI> gamFilter(-1,
...
bool RealTimeStateTest::TestAddStatefulsFalse_InvalidStateful() {
	...
	ReferenceT<RealTimeStateStateLessGAM> notStateful =
			ObjectRegistryDatabase::Instance()->Find(
					"Fibonacci.Functions.GAMGroup1.GAMB");
...
bool RealTimeStateTest::TestPrepareNextState() {
	...
	ReferenceT<RealTimeStateStateFulGAM> gama = functions->Find(
			"GAMGroup1.GAMA");
	ReferenceT<RealTimeStateStateLessGAM> gamb = functions->Find(
			"GAMGroup1.GAMB");
	ReferenceT<RealTimeStateStateLessGAM> gamc = functions->Find(
			"GAMGroup1.GAMGroup2.GAMC");
	ReferenceT<RealTimeStateStateLessGAM> gamd = functions->Find(
			"GAMGroup1.GAMGroup2.GAMD");
	ReferenceT<RealTimeStateStateLessGAM> game = functions->Find(
			"GAMGroup3.Container.GAMGroup4.GAME");
	ReferenceT<RealTimeStateStateLessGAM> gamf = functions->Find(
			"GAMGroup3.Container.GAMF");
	ReferenceT<RealTimeStateStateLessGAM> gamg = functions->Find(
			"GAMGroup3.GAMG");
	ReferenceT<RealTimeStateStateFulGAM> gamh = functions->Find(
			"GAMGroup3.GAMH");
