...
static bool CompareConfigurationStrings(const char8 * const str1,
                                        const char8 * const str2) {
    const char8 *str1Buf = str1;
    const char8 *str2Buf = str2;
    bool ok = true;
    bool end = false;
    uint32 lineNumber = 1u;
    while (!end) {
        while ((*str1Buf == ' ') || (*str1Buf == '\n')) {
            //Just to debug...
            if (*str1Buf == '\n') {
                lineNumber++;
            }
        ...
        while ((*str2Buf == ' ') || (*str2Buf == '\n')) {
            str2Buf++;
        }
        ok = (*str1Buf == *str2Buf);
        if (!ok) {
            printf("Mismatch found @ line [%d]: [", lineNumber);
            StreamString s1 = str1;
            StreamString line;
            s1.Seek(0);
            while (lineNumber > 0u) {
                line = "";
    ...
    //Check if there were trailing characters to check
    while ((*str1Buf != '\0') && (ok)) {
        ok = ((*str1Buf == ' ') || (*str1Buf == '\n'));
        str1Buf++;
    }
...
static bool TestBuilder(const char8 * const config,
    ...
    if (ret) {
        ret = rtAppBuilder.Copy(fcdb, dcdb);
    }
    ...
    printf("%s\n", dDisplay.Buffer());
    if (ret) {
        ret = CompareConfigurationStrings(fDisplay.Buffer(), expectedFunctionsConfig);
    }
    if (ret) {
        ret = CompareConfigurationStrings(dDisplay.Buffer(), expectedDataConfig);
    }
    if (!ret) {
